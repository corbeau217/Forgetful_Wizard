...
public class CellPlacementRules
{
    ...
    public CellPlacementRules( Color[] filledMask, Color[] adjacencyMask, Color[] vacancyMask ){
        ...
        for (int zoneIndex = 0; zoneIndex < 9; zoneIndex++) {
            // zone index row major order starting from top left
            //  pixel index is row major order starting from bottom left
            int pixelIndex = zoneIndexToPixelIndex(zoneIndex);
            ...
            this.filledRequired[zoneIndex] = Mathf.Abs((filledVec.magnitude) - (fullVec.magnitude)) < this.maskTollerance; 
            this.adjacencyRequired[zoneIndex] = Mathf.Abs((adjacencyVec.magnitude) - (fullVec.magnitude)) < this.maskTollerance; 
            this.vacancyRequired[zoneIndex] = Mathf.Abs((vacancyVec.magnitude) - (fullVec.magnitude)) < this.maskTollerance; 
        }
    }

    // converts zone to texture's pixel index
    //  zone index start from the top left in row major order
    //  texture pixel indices start bottom left also in row major order 
    int zoneIndexToPixelIndex(int zoneIndex){
        // return zoneIndex;
        int columnCount = 3;
        int rowCount = 3;

        // extract column index, stays same
        int zoneColIndex = zoneIndex % columnCount;
        
        // extract zone row index
        int zoneRowIndexTopToBottom = zoneIndex / columnCount;

        // convert to pixel row index
        int bottomCellRow = (rowCount-1);
        int pixelRowIndex = bottomCellRow - zoneRowIndexTopToBottom;

        // combine for the pixel index
        return (pixelRowIndex*columnCount)+zoneColIndex;
    }
